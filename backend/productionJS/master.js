...
function setStateFromPath(path){
  ...
  if(ar.includes('shop') && !window.location.search){
    let focused = document.querySelectorAll('.focus:not(.reduce)')[0],
        theme = (focused.dataset.name) ? focused.dataset.name : '',
        page = focused.dataset.address,
        item = (ar[4]) ? ar[4] : '',
        title = theme + ' ' + item;
    updateState('shop', page, theme, item, title, path);
  }
}
...
function MakeScroller2(){
  ...
  this.build = function(p, links){
    ...
    for (let i = 0; i < links.length; i++) {
      ...
      let source = (window.location.hostname === 'www.porchespottery.com') ?
          window.location.hostname : 'https://potterytest.endurance.link';
          img.setAttribute('src', source + links[i]);
          img.setAttribute('alt', source + links[i]);
          img.classList.add('smooth', 'imgScroller');

      imgDivs.push(img);
      if(base.children[i]){
        if(links.length === 1){
          base.children[1].appendChild(img);
        }
        else{
          base.children[i].appendChild(img);
        }
      }
    }
...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  if(window.location.pathname.includes('/shop/designs')){
    let designs = Array.from(document.querySelectorAll('.fetch'));
    preLoadItems(designs, '/stockQuery');
  }
      ...
      xhr({ [items[i].dataset.fetchwhat] : items[i].dataset.fetchthis}, route, function(res){
        ...
        if(i === items.length -1){
          ...
          if(path[4]){
            ...
            target = document.querySelector('.itemModal[data-item="' + item +'"][ data-theme="' + theme +'"]');
  ...
  if(document.querySelector('[data-name="'+ target.dataset.fetchthis+'"]' ).innerHTML.length < 10){
    xhr({ [target.dataset.fetchwhat] : target.dataset.fetchthis}, route, function(res){
      let targetPage = document.querySelector('[data-name="'+ target.dataset.fetchthis+'"]' );
          targetPage.innerHTML = res;
          url = new URL(window.location.href + '/' + target.dataset.fetchthis.replace(/ /gi, '_'));
...
function openModalSetState(page, targetModal, themeOrItem){
  ...
  targetModal.querySelector('.plus').addEventListener('click', function(){
  ...
  //why is it 5?
  let add = (len < 5) ? window.location.pathname + '/' + themeOrItem : window.location.pathname;
  setStateFromPath(add);
}
